...
* **Live USPS Time Clock:** A sleek, live-updating display in the header shows the current date, standard 24-hour time (with seconds), and the special USPS time (24-hour with minutes in hundredths), all based on the user's local timezone.
...
* **Acronyms Database:** A searchable and sortable reference for common USPS acronyms, endorsements, and forms.
...
* **Display Options:** All customization features are located within the "Display Options" accordion menu, which contains several nested sections.
...
This guide is an internal tool providing essential information for a mail carrier's daily duties. It's organized into collapsible sections for easy navigation, including **Change of Address**, **Hold Mail**, **Informed Delivery**, and a list of common carrier forms.
...
This page contains links to external websites and internal tools like the Time Conversion Table. External links will open in a new tab.
...
* `data/`: This directory holds all the application's data in JSON format.
...
* **Local Storage:** User preferences for the theme, carrier selections, event filters, custom colors, T6 routes, and letter schedules are saved in the browser's local storage for a persistent experience.
